...
final private[xchange_spray_json] case class CurrencyBalance(currency: Option[Currency], balance: Option[Balance])
final private[xchange_spray_json] case class AccountWallet(account: Option[String], wallet: Option[Wallet])
...
trait XChangeJsonFormats extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  private def removeJsNulls(jsObject: JsObject): JsObject =
  ...
  implicit val BalanceFormat = new JsonFormat[Balance] {
    ...
    override def write(x: Balance): JsObject =
      removeJsNulls(
        JsObject(
          Map(
  ...
  implicit private val CurrencyBalanceFormat = jsonFormat2(CurrencyBalance)

  implicit val WalletFormat = new JsonFormat[Wallet] {

    override def write(x: Wallet): JsObject =
      removeJsNulls(
        JsObject(
          Map(
  ...
  implicit private val AccountWalletFormat = jsonFormat2(AccountWallet)

  implicit val AccountInfoFormat = new JsonFormat[AccountInfo] {

    override def write(x: AccountInfo): JsObject =
      removeJsNulls(
        JsObject(
          Map(
  ...
  implicit val LimitOrderFormat = new JsonFormat[LimitOrder] {
    ...
    override def write(x: LimitOrder): JsObject =
      removeJsNulls(
        JsObject(
          Map(
  ...
  implicit val UserTradeFormat = new JsonFormat[UserTrade] {
    ...
    override def write(x: UserTrade): JsObject =
      removeJsNulls(
        JsObject(
          Map(
  ...
  implicit val UserTradesFormat = new JsonFormat[UserTrades] {
    ...
    override def write(x: UserTrades): JsObject =
